...
const handleSavePdf_backend = async () =>{
  ...
  formData.append("deleted_page_no",document.getElementById("dl_pg").value.trim());

  try{
    const response = await fetch(`${API_URL}/api/delete-pages`, {
      method: "POST",
      body: formData,
    });
    ...
    if (!response.ok){
       ...
       throw new Error("Deleted Pages action failed");
     }
